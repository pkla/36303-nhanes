...
```{r}
# Create age groups
merged[(merged$RIDAGEYR >= 16) & (merged$RIDAGEYR < 35), "age.group"] <- "16to35"
merged[(merged$RIDAGEYR >= 35) & (merged$RIDAGEYR < 65), "age.group"] <- "35to65"
...
ggsurvey(merged.survey, aes(x=RIDAGEYR, y=sleep.time.weekday.offset)) + geom_point(alpha=0.01) +
  labs(title="Sleep time (Weekdays or workdays)")

ggsurvey(merged.survey, aes(x=RIDAGEYR, y=wake.time.weekday.offset)) + geom_point(alpha=0.01) +
  labs(title="Wake time (Weekdays or workdays)")

ggsurvey(merged.survey, aes(x=RIDAGEYR, y=sleep.time.weekend.offset)) + geom_point(alpha=0.01) +
  labs(title = "Sleep time (Weekend)")
ggsurvey(merged.survey, aes(x=RIDAGEYR, y=wake.time.weekend.offset)) + geom_point(alpha=0.01) +
  labs(title="Wake time (Weekend)")

ggsurvey(merged.survey, aes(x=RIDAGEYR, y=sleep.hours.weekday)) + geom_point(alpha=0.01) +
  labs(title="Sleep Hours (Weekdays or Workdays)")
ggsurvey(merged.survey, aes(x=RIDAGEYR, y=sleep.hours.weekend)) + geom_point(alpha=0.01) +
  labs(title="Sleep Hours (Weekends)")
```



```{r}
ggsurvey(merged.survey, aes(x=age.group, y=sleep.hours.weekend)) + geom_boxplot(alpha=0.01) +
  labs(title="Sleep time (Weekend)")

ggsurvey(merged.survey, aes(x=age.group, y=sleep.hours.weekday)) + geom_boxplot(alpha=0.01) +
  labs(title="Sleep time (Weekdays or workdays)")
...
```{r}
svychisq(~sleep.hours.weekday + age.group, merged.survey)
```



```{r}
svychisq(~sleep.hours.weekend + age.group, merged.survey)
```

```{r}
summary(svyglm(sleep.hours.weekend ~ RIDAGEYR, merged.survey))
```
...
```{r}
summary(svyglm(sleep.hours.weekday ~ RIDAGEYR, merged.survey))
```
