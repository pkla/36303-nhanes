...
```{r}
# Set missing values and codes to NA
merged[!is.na(merged[, "sleep.time.weekday"] == "77777"), "sleep.time.weekday"] <- NA
merged[!is.na(merged[, "sleep.time.weekday"] == "99999"), "sleep.time.weekday"] <- NA

merged[!is.na(merged[, "wake.time.weekday"] == "77777"), "wake.time.weekday"] <- NA
merged[!is.na(merged[, "wake.time.weekday"] == "99999"), "wake.time.weekday"] <- NA

merged[!is.na(merged[, "sleep.time.weekend"] == "77777"), "sleep.time.weekend"] <- NA
merged[!is.na(merged[, "sleep.time.weekend"] == "99999"), "sleep.time.weekend"] <- NA

merged[!is.na(merged[, "wake.time.weekend"] == "77777"), "wake.time.weekend"] <- NA
merged[!is.na(merged[, "wake.time.weekend"] == "99999"), "wake.time.weekend"] <- NA

merged[!is.na(merged[, "frequency.snore"] == 7), "frequency.snore"] <- NA
merged[!is.na(merged[, "frequency.snore"] == 9), "frequency.snore"] <- NA

merged[!is.na(merged[, "frequency.stop.breathing"] == 7), "frequency.stop.breathing"] <- NA
merged[!is.na(merged[, "frequency.stop.breathing"] == 9), "frequency.stop.breathing"] <- NA

merged[!is.na(merged[, "told.doctor.sleep.trouble"] == 9), "told.doctor.sleep.trouble"] <- NA
merged[!is.na(merged[, "frequency.daytime.fatigue"] == 9), "frequency.daytime.fatigue"] <- NA
```


```{r}
# Create age groups
merged$age16to30 <- merged$RIDAGEYR < 30
merged$age30to50 <- merged$RIDAGEYR < 50
merged$age50to70 <- merged$RIDAGEYR < 70
merged$age70plus <- merged$RIDAGEYR >=70
```
